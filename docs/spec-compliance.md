...
| Top-level `links.describedby` (link to a description document) | ✅ test | First-class on `Schema\Link\DocumentLinks` (named `describedby` param + getter); `Response\AbstractResponse::withDescribedby()` merges it into the rendered top-level `links` at render time (symmetric with the by-convention `self`), so it coexists with a handler's `self`/pagination/custom links and an author-set `describedby` wins. The Symfony bundle wires it to the served OpenAPI spec URL. `DocumentLinksTest::describedbyIsAFirstClassNamedMember`, `DataResponseTest::withDescribedbyMergesADescribedbyLinkAlongsideSelf`. |
...
| Local identifiers (`lid`) on resource objects / resource identifiers (creation) | ✅ test | `ResourceIdentifier` carries `?id`/`?lid` (`fromArray()` requires `type` + at-least-one-of; `ResourceIdentifierTest`); a relationship referenced by `lid` hydrates through to the relationship hydrator (`CreateHydratorTraitTest`); the request exposes `getResourceLid()`. **Scope boundary:** `lid` is only meaningful within an [Atomic Operations](#atomic-operations-extension-specatomic-operations) batch, where it is resolved by `Atomic\LocalIdRegistry`. A `lid` **outside** an atomic batch — on a plain create/update primary resource or in its embedded linkage — is rejected `400 LOCAL_ID_NOT_SUPPORTED` (`JsonApiRequest` → `LocalIdNotSupported`), because there is no batch to resolve it against. `JsonApiRequestTest` (`spec:atomic-operations`). |
...
| `ext` parameter negotiation (415/406 on unsupported extensions) | ✅ test | `RequestValidator(string ...$supportedExtensions)` rejects an `ext` not in its supported set: 415 on `Content-Type`, 406 on `Accept`. **The default supported set is empty**; a server opts an extension in explicitly. The **Atomic Operations** extension ships (see [its section below](#atomic-operations-extension-specatomic-operations)) and, when enabled, is a member of the supported set. `RequestValidatorTest`, `ExtensionTest`. |
...
| Extensions cannot be used unless server-supported (415/406) | ✅ test | See the content-negotiation `ext` row. The negotiation path is wired and tested against both the empty supported set and the atomic set; the **Atomic Operations** extension ships (see [its section below](#atomic-operations-extension-specatomic-operations)) and is negotiated only when a server enables it. `RequestValidatorTest`, `ExtensionTest`. |

### Atomic Operations extension (`spec:atomic-operations`)

The [Atomic Operations extension](https://jsonapi.org/ext/atomic/) (URI
`https://jsonapi.org/ext/atomic`) ships as an **opt-in, default-off** capability: a
server enables it by adding the URI to its supported extension set, after which a
`POST` of an `atomic:operations` document runs the batch. Each requirement below is
implemented and tested.

| Requirement | Status | Notes |
|---|---|---|
| Extension URI + document members | ✅ test | `Atomic\AtomicExtension` defines the `URI`, the request member `atomic:operations` and the response member `atomic:results`. `AtomicExtensionTest`. |
| Operation objects (`op` = `add`/`update`/`remove`; `ref`/`href` target; `data`) parsed and validated | ✅ test | `Atomic\AtomicOperationsParser` requires an `atomic:operations` array, a known `op`, and exactly one target form; a malformed batch renders a spec-shaped `400`. `AtomicOperationsParserTest`. |
| Operations executed **in document order**, all-or-nothing | ✅ test | `Atomic\AtomicLoop` runs each operation in order over one transactional backend, committing on success and rolling back on any failure. `AtomicLoopTest::commitsAndReturnsResultsInOrderOnSuccess`, `::rollsBackAndReturnsAPrefixedErrorWhenAnOperationThrows`, `::rollsBackAndReturnsAnErrorWhenTheCommitThrows`. |
| `atomic:results` array; an empty result is a result object `{}` | ✅ test | `Atomic\AtomicResult` (`empty()` / `fromDocument()`); the response pins the wire type of an empty result as the JSON object `{}`, never `[]`. `AtomicResultsResponseTest::rendersTheResultsArrayWith200AndTheExtensionContentType`. |
| Errors located at `/atomic:operations/<index>…` | ✅ test | `AtomicLoop` prefixes a failing operation's inner `source.pointer` with `/atomic:operations/<i>`; a pointerless inner error locates at the operation itself; a `source.parameter`-only error is preserved untouched. `AtomicLoopTest::rollsBackAndReturnsAPrefixedErrorWhenAnOperationThrows`, `::locatesAPointerlessErrorAtTheOperationItself`, `::preservesAParameterOnlySourceWithoutAddingAPointer`. |
| Local identifiers (`lid`) resolved within the batch | ✅ test | `Atomic\LocalIdRegistry` records the id a creating operation assigns to its `lid` and resolves later references to it; the same registry instance threads through every operation. `LocalIdRegistryTest`, `AtomicLoopTest::threadsTheSameRegistryInstanceToEveryExecuteOne`. (Outside an atomic batch a `lid` is rejected `400 LOCAL_ID_NOT_SUPPORTED` — see the local-identifiers row under [CRUD](#crud-speccrud-speccreating-resources-specupdating-resources-specupdating-relationships).) |
| Applied extension advertised on the response | ✅ test | The results response and any rolled-back error response carry `ext="https://jsonapi.org/ext/atomic"` on `Content-Type` and the URI in top-level `jsonapi.ext`. `AtomicResultsResponseTest`, `AtomicLoopTest::theRolledBackErrorResponseAdvertisesTheAtomicExtension`. |
