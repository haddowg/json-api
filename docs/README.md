...
- [Architecture](architecture.md) — how the layers (request, schema, engine, response) fit together.
- [Spec compliance](spec-compliance.md) — the canonical JSON:API 1.1 MUST/SHOULD coverage table.

## Defining resources

- [Schemas](schemas.md) — the recommended way to declare a resource type (one list drives both directions).
...
- [Resources](resources.md) — the hand-written serializer escape hatch.
...
- [Adapters](adapters.md) — bridging the operations layer to a framework's HTTP stack.
- [Testing](testing.md) — assertion wrappers, request/operation builders, and the spec-compliance check.
- [Upgrading within 0.x](upgrading-within-0.x.md) — pre-1.0 breaking-change policy and the migration log.
